# SnR null model RWR generation and testing against observed LCBD
# Dataset: Prediction from RF diversity models
# Author: NJ
# Date: 18.8.25 (updated to mirror observed RWR script)
...
source("02_Analysis/01_Scripts/sources/Shift&RotFunctions.R")   # provides shiftrotXY()
...
# ---- Load data for observed model (follow script 20 style) ----
...
predictor_masked <- rast("01_Data/02_Clean/predictor_stack_EPSG32719_250m.tif")

# ---- Load observed model prediction rasters ----
lcbd_path <- "02_Analysis/02_Results/02_Predictions/LCBD_prediction_250m.tif"
rwr_path  <- "02_Analysis/02_Results/02_Predictions/RWR_prediction_250m.tif"
...
# Add class_code raster factorization like in RF model scripts
...
# Remove all-zero species (avoid 1/0-inf)
species <- species[, colSums(species, na.rm = TRUE) > 0, drop = FALSE]

# Convert species data to presence/absence and calculate RWR
species_pa <- decostand(species, method = "pa")
sp_freq     <- colSums(species_pa)
sp_weight   <- 1 / sp_freq
...
# Pull RWR vector from the thinned dataset
RWR <- env$RWR

# ---- Step 4: Feature selection ----
# Use pre-selected variables from RWR modeling (7 vars from script 07)
chosenvars1 <- c("ndvi_max", "mean_temp", "annual_precip", "temp_range", "canopy_height", "class_code", "dist_pw")
print(chosenvars1)

# ---- Step 5: Spatial model ----
# Preparation of training and testing data
joint_data <- cbind(env, RWR = RWR)
colnames(joint_data)[ncol(joint_data)] <- "RWR"

# Stratify folds by class_code (not by RWR values)
set.seed(42)
group_rwr <- createFolds(joint_data$class_code, k = 6)

# Combine indices for training 
train_indices <- unlist(group_rwr[2:6])
test_indices <- group_rwr[[1]]

# Add the RWR variable to the predictors
joint_vars <- c("RWR", chosenvars1)

# Define the variables to use for each dataset
rwr_train <- joint_data[train_indices, joint_vars]
rwr_test <- joint_data[test_indices, joint_vars]

# Ensure class_code is factor in both splits
...
# Add class weighting (inverse frequency weighting)
rf_weights <- NULL
if ("class_code" %in% names(rwr_train)) {
...
set.seed(42)
rf_model <- randomForest(RWR ~ ndvi_max + mean_temp + annual_precip + temp_range + canopy_height + class_code + dist_pw,
                         data = rwr_train,
                         importance = TRUE,
                         ntree = 500,
                         weights = rf_weights)
...
# Performance metrics
predicted <- predict(rf_model, newdata = rwr_test)
observed <- rwr_test$RWR
...
# ---- Step 7: S&R nulls + LCBD vs RWR test ----
...
# Use original raster extent (no torus wrapping for masked study areas)
pred_src <- pred_base  # Use original raster directly
plot(pred_src[[1]]) # sanity check
...
# Observed rasters - ensure they match predictor_masked geometry
r_lcbd <- terra::rast(lcbd_path)
r_rwr  <- terra::rast(rwr_path)
...
if (!terra::compareGeom(r_rwr, predictor_masked, stopOnError = FALSE)) {
  cat("Resampling RWR raster to match predictor_masked geometry...\n")
  r_rwr <- terra::resample(r_rwr, predictor_masked, method = "bilinear")
}

# Common mask: areas where all three rasters have data
msk <- !is.na(r_lcbd) & !is.na(r_rwr) & !is.na(predictor_masked[[1]])
r_lcbd <- terra::mask(r_lcbd, msk)
r_rwr  <- terra::mask(r_rwr,  msk)
cat("Valid pixels after masking:", sum(terra::values(msk), na.rm = TRUE), "\n")

# Observed vectors & stats
v_lcbd      <- as.vector(r_lcbd[])
v_rwr       <- as.vector(r_rwr[])
qs          <- seq(0.05, 0.50, by = 0.05)
obs_rho     <- suppressWarnings(cor(v_lcbd, v_rwr, method = "spearman", use = "complete.obs"))
obs_overlap <- overlap_curve(v_lcbd, v_rwr, qs)

# Grid to predict on (same geometry as observed rwr raster)
template <- r_rwr
grid_xy  <- terra::crds(template, df = TRUE)

# tiny rigid-transform helpers
estT <- function(X,Y){X<-as.matrix(X);Y<-as.matrix(Y);mx<-colMeans(X);my<-colMeans(Y)
...
applyT <- function(P,T){sweep(as.matrix(P),2,T$mx,"-")%*%T$R + matrix(T$my,nrow(P),2,byrow=TRUE)}

# ---- Step 6: Build S&R nulls with SAME vars, splits, and weighting ----           # <<< CHANGED
nsim_snr <- 999
set.seed(42)
null_rho <- numeric(nsim_snr)
null_ovl <- matrix(NA_real_, nrow = nsim_snr, ncol = length(qs))
...
for (i in seq_len(nsim_snr)) {
  ...
  # CRITICAL: Use SAME RWR values as observed (not spatially transformed)
  # This maintains the null hypothesis that spatial structure doesn't matter
  site_df$RWR <- env$RWR
  ...
  # Use SAME train indices as observed model (critical for proper null hypothesis)
  ...
  rf_null_i <- randomForest(
    RWR ~ ndvi_max + mean_temp + annual_precip + temp_range + canopy_height + class_code + dist_pw,
    data = null_train_data, ntree = 500, importance = FALSE, weights = null_weights
  )
  
  # 3) apply SAME transform to grid, extract predictors, predict
...
png("02_Analysis/03_Figures/Original vs SnR points RWR.png")
par(mfrow = c(1,1))
plot(pred_src[[1]], colNA = "grey90")
points(coord[,1],   coord[,2],   pch = 1,  col = "black")
points(coordR[,1],  coordR[,2],  pch = 16, col = "red")
legend("bottom", c("original","rotated"), pch = c(1,16), col = c("black","red"))
dev.off()

# ---- Envelopes + p-values ----
p_rho_attraction <- (1 + sum(null_rho >= obs_rho)) / (nsim_snr + 1)

null_mean <- apply(null_ovl, 2, mean, na.rm = TRUE)
null_lo   <- apply(null_ovl, 2, quantile, probs = 0.025, na.rm = TRUE)
null_hi   <- apply(null_ovl, 2, quantile, probs = 0.975, na.rm = TRUE)

sd_q        <- apply(null_ovl, 2, sd, na.rm = TRUE)
z_sum_obs   <- sum((obs_overlap - null_mean) / sd_q, na.rm = TRUE)
z_sum_null  <- apply((null_ovl - matrix(null_mean, nrow = nsim_snr, ncol = length(qs), byrow = TRUE)) / matrix(sd_q, nrow = nsim_snr, ncol = length(qs), byrow = TRUE),
                     1, function(z) sum(z, na.rm = TRUE))
p_curve     <- (1 + sum(z_sum_null >= z_sum_obs)) / (nsim_snr + 1)
...
snr_results <- list(
  ...
  p_rho_attraction = p_rho_attraction,
  ...
  # Metadata
  nsim = nsim_snr,
  analysis_type = "LCBD_vs_RWR",
  date = Sys.Date()
)

# Save as RDS (preserves all R data structures)
saveRDS(snr_results, file.path(out_dir_res, "SnR_nulls_results_RWR.rds"))
...
write.csv(rho_results, file.path(out_dir_res, "SnR_nulls_rho_RWR.csv"), row.names = FALSE)
...
write.csv(overlap_results, file.path(out_dir_res, "SnR_nulls_overlap_RWR.csv"), row.names = FALSE)

# Save summary statistics as CSV
summary_stats <- data.frame(
  metric = c("obs_rho", "null_rho_mean", "null_rho_sd", "p_rho_attraction", 
             "p_curve_global", "nsim", "valid_nulls_rho", "valid_nulls_overlap"),
  value = c(obs_rho, mean(null_rho, na.rm = TRUE), sd(null_rho, na.rm = TRUE), 
            p_rho_attraction, p_curve, nsim_snr, 
            sum(!is.na(null_rho)), sum(apply(null_ovl, 1, function(x) any(!is.na(x))))),
  stringsAsFactors = FALSE
)
write.csv(summary_stats, file.path(out_dir_res, "SnR_nulls_summary_stats_RWR.csv"), row.names = FALSE)
...
write.csv(quantile_comparison, file.path(out_dir_res, "SnR_nulls_quantile_comparison_RWR.csv"), row.names = FALSE)

cat("Results saved to:\n")
cat("- RDS:", file.path(out_dir_res, "SnR_nulls_results_RWR.rds"), "\n")
cat("- CSV files:", file.path(out_dir_res, "SnR_nulls_*_RWR.csv"), "\n")

# ---- Plots + summary ----
save_hist(
  x = null_rho, x_obs = obs_rho,
  title = "S&R nulls — rank correlation (LCBD vs RWR)",
  subtitle = sprintf("obs rho = %.3f; p(attraction) = %.4f; nsim = %d", obs_rho, p_rho_attraction, nsim_snr),
  xlab = "Spearman rho",
  file = file.path(out_dir_fig, "SnR_nulls_rho_hist_RWR.png")
)
...
save_curve(
  ...
  subtitle = sprintf("Global p (higher-than-expected overlap) = %.4f; nsim = %d", p_curve, nsim_snr),
  xlab = "Top-quantile threshold",
  ylab = "Jaccard overlap",
  file = file.path(out_dir_fig, "SnR_nulls_overlap_curve_RWR.png")
)

sink(file.path(out_dir_res, "SnR_nulls_summary_RWR.txt"))
cat("S&R nulls (in-memory) - RWR vs LCBD\n")
cat("n null rasters:", nsim_snr, "\n")
cat(sprintf("Observed Spearman rho (LCBD vs observed RWR): %.4f\n", obs_rho))
cat(sprintf("p(attraction, rho): %.4f\n", p_rho_attraction))
cat("Quantiles: ", paste(qs, collapse = ", "), "\n")
cat(sprintf("Global p (overlap curve, higher-than-expected): %.4f\n", p_curve), "\n")
sink()
...
p1 <- ggplot(df_rho, aes(null_rho)) +
  ...
  annotate("text", x = min(null_rho, na.rm = TRUE), 
           ...
           label = sprintf("p = %.4f\nnsim = %d", p_rho_attraction, nsim_snr)) +
  labs(x = "Spearman rho", y = "Count") +
  theme_minimal()

ggsave(file.path(out_dir_fig, "SnR_nulls_rho_pub_RWR.png"), p1, width = 7, height = 5, dpi = 300)

df_plot <- data.frame(q = qs, obs = obs_overlap, lo = null_lo, hi = null_hi, mean = null_mean)
...
ggsave(file.path(out_dir_fig, "SnR_nulls_overlap_curve_pub_RWR.png"), p2, width = 7, height = 5, dpi = 300)

# ---- Comments ----
# This version follows script 20 methodology but adapted for RWR:
# - spatial thinning with spThin (100m minimum distance)
# - stratified folds by class_code (not by RWR values)
# - pre-selected variables from RWR modeling (no RFE)
# - weighted RF (inverse class frequency) for observed + each null
# - 999 simulations for full S&R null analysis
# - class_code raster factorization like in RF model scripts
